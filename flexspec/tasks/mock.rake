...
namespace :mock do
  task :build do
    ...
    rm_r(mock_config.output_dir) if File.exist?(mock_config.output_dir)
    FileUtils.mkdir_p(mock_config.output_dir)

    asdoc_output = %x(asdoc -source-path #{mock_config.source_path} -doc-sources #{mock_config.doc_sources} -external-library-path+=#{mock_config.external_library_path} -templates-path #{mock_config.templates_path} -output #{mock_config.output_dir})
